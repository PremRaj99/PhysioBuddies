...
export default function BlogCard({
  ...
  const eclipseDesc =
    descr.length > 120 ? descr.substring(0, 120) + "..." : descr;
  ...
  return (
    <div className="w-96 border border-gray-300 rounded-lg hover:scale-[1.01] shadow-lg duration-150 cursor-pointer" onClick={onClick}>
      ...
      />
      <div className="p-2">
        <h3 className="text-lg  font-semibold">{eclipseTitle}</h3>
        <p className="text-sm font-thin text-gray-800">{eclipseDesc}</p>
        <div className="text-xs pt-2 font-semibold flex justify-between items-center">
          {dateFormat}
           <div className="flex items-center gap-2"> <FaEye /> <span>{views}</span></div>
        </div>
      </div>
    </div>
  );
